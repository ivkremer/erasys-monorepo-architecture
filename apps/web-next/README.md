...
This is a simple Next.js app with Tailwind CSS which uses the [profile-pictures](../../packages/profile-pictures)
package.

[See on prod](https://erasys-monorepo-architecture-web-ne.vercel.app/) (Hosted on Vercel).
...
While there are defaulted URLs for `profile-pictures`, you can specify your own with the local `.env` as follows:
...
The profile slug (like `"msescortplus"`) can also be sourced from `.env`. See the [`.env.example`](./.env.example).
